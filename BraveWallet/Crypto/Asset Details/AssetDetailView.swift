          ...
          ForEach(assetDetailStore.accounts) { viewModel in
            HStack {
              ...
              VStack(alignment: .trailing) {
                Text(verbatim: showFiatPlaceholder ? "$0.00" : viewModel.fiatBalance)
                  .redacted(reason: showFiatPlaceholder ? .placeholder : [])
                  .shimmer(assetDetailStore.isLoadingPrice)
                Text(verbatim: showBalancePlaceholder ? "0.0000 \(assetDetailStore.token.symbol)" : "\(viewModel.balance) \(assetDetailStore.token.symbol)")
...
struct CurrencyDetailView_Previews: PreviewProvider {
  static var previews: some View {
    ...
    .previewColorSchemes()
    .environmentObject(WalletStore.previewStore)
  }
}
#endif
